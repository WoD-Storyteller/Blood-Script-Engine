...
export class ResolutionPipeline {
  ...
  async run(
    ...
    // ─────────────────────────────────────────────
    // 0) Presence gating (no resolving against offline players)
    // ─────────────────────────────────────────────
    if (input.mentionedDiscordUserIds.length > 0) {
      const offline = await this.findOfflineTargets(
        client,
        input.engineId,
        input.mentionedDiscordUserIds,
      );
      ...
      if (offline.length > 0) {
        return {
          ...
          publicMessage:
            'That cannot resolve right now — one or more involved players are not present.',
        };
      }
    }
    ...
    // 1) Tenet enforcement
    // ─────────────────────────────────────────────
    const tenetCheck: TenetCheckResult =
      await this.safety.checkTenets(client, {
    ...
    if (tenetCheck.allowed === false) {
      ...
      return {
        ...
        publicMessage:
          'That direction is outside this chronicle’s boundaries. The night moves elsewhere.',
      };
    }
    ...
    // ─────────────────────────────────────────────
    // 3) Discipline detection (rouse + hunger)
    // ─────────────────────────────────────────────
    const detected = this.disciplines.detect(input.content);
    if (detected && character) {
      const dots = await this.charCtx.getDisciplineDots(
        client,
    ...
    // ─────────────────────────────────────────────
    const isCombat =
      /attack|strike|shoot|stab|fight|hit|slash|bite/i.test(
        input.content,
      );
    ...
    if (isCombat && input.mentionedDiscordUserIds.length > 0) {
      ...
      await client.query(
        `UPDATE scenes SET updated_at = now() WHERE scene_id = $1`,
        [input.sceneId],
      );
  ...
  private async findOfflineTargets(
    client: any,
    engineId: string,
    ...
    const status = new Map(
      presence.rows.map((r: any) => [r.character_id, r.status]),
    );

    return charIds.filter(
      (id) => !status.has(id) || status.get(id) === 'offline',
    );
  }
}
